...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    public Iterable<Edge<V,E>> incomingEdges(V vert){
        ...
        for(Vertex<V,E> vertex: vertices.values()){
           if(!vertex.equals(vert)){
               for(Edge<V,E> edge: outgoingEdges(vertex.getElement())){
    ...
    public boolean removeVertex(V vert){
        ...
        for(Edge<V,E> edge: incomingEdges(vert)){
            removeEdge(edge.getVOrig(), edge.getVDest());
        }
        
        int key = vertices.get(vert).getKey();
        ...
        if(vertices.containsKey(vert))
            vertices.remove(vertices.get(vert).getElement());
